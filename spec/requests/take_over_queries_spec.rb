...
RSpec.describe 'Take over queries', type: :request do # rubocop:disable Metrics/BlockLength
  example 'delete' do
  ...
  example 'list page' do
    ...
    # edit link
    expect(doc.css('.ds-list-row .spec-ds-edit').first.attribute('href').value)
      .to eq('/en/request/countries/1/edit?foo=bar&page=1')

    # delete button
    expect(doc.css('.ds-list-row .js-ds-destory').first.attribute('data-action').value)
      .to eq('/en/request/countries/1?foo=bar')
  end

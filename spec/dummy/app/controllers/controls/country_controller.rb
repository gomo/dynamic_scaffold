module Controls
  class CountryController < ApplicationController
    ...
    dynamic_scaffold Country do |s|
      ...
      s.list.item('Update Date', style: 'width: 162px;') do |record|
        record.fdate(:updated_at, '%Y-%m-%d %H:%M:%S')
      end
      s.list.item('Create Date', style: 'width: 162px;') do |record|
        record.fdate(:created_at, '%Y-%m-%d %H:%M:%S')
      end
      ...
      s.editor.text_field(:name).note do
        content_tag :p do
          out = []
